...
export default {
  ...
  data() {
    ...
    return {
      sdk: {
        ...
        environment: SDKEnvironmentTypes.UAT,
      },
    };
  },
  ...
  methods: {
    handleError(error) {
      ...
      alert(`Ocorreu um erro com o SDK: ${error.message || 'Erro desconhecido. Verifique o console.'}`);
    },
    ...
    instantiateNewBuilder() {
      try {
        ...
        this.handleError(new Error(`Failed to instantiate UnicoCheckBuilder: ${err.message || err}`));
      }
    },
    ...
    instantiateTheme() {
      try {
        ...
        console.log("UnicoThemeBuilder theme built.");
         ...
         this.handleError(new Error(`Failed to build theme: ${err.message || err}`));
      }
    },

    async instantiateCamera() {
      if (!this.sdk.builder) {
          this.handleError(new Error("SDK Builder not instantiated before trying to instantiate camera."));
          return;
      }
       if (!this.sdk.theme) {
          this.handleError(new Error("SDK Theme not instantiated before trying to instantiate camera."));
          return;
      }

      try {
        console.log("Instantiating camera with environment:", this.sdk.environment);
        this.sdk.builder
          .setTheme(this.sdk.theme)
          .setModelsPath(this.sdk.models)
          .setResourceDirectory(this.sdk.resources)
          .setEnvironment(this.sdk.environment);

        this.sdk.camera = await this.sdk.builder.build();
        console.log("Unico Camera instantiated successfully.");

      } catch (err) {
        this.handleError(new Error(`Failed to instantiate camera (build step): ${err.message || err}`));
      }
    },
    ...
    async loadSDK() {
      ...
      if (this.sdk.builder && this.sdk.theme) {
         await this.instantiateCamera();
          ...
          console.error("Cannot instantiate camera because builder or theme failed.");
      }
      console.log("SDK Load process finished.");
    },

    async prepareCamera(cameraType, prepareMethod, successCallback, errorCallback) {
       if (!this.sdk.camera) {
        this.handleError(new Error("Camera not ready. Load SDK process might have failed."));
        return;
      }
      const vm = this;
      ...
      try {
        ...
        const cameraOpener = await prepareMethod(
            this.sdk.services
        );
        ...
        cameraOpener.open({
          on: {
            success: (data) => {
              console.log(`${cameraType} Success:`, data.base64 ? data.base64.substring(0, 50) + "..." : data);
            ...
            support: (msg) => console.log(`${cameraType} Support Message:`, msg),
          },
        });
      } catch (e) {
        console.error(`Error preparing ${cameraType}:`, e);
        vm.handleError(new Error(`Erro ao preparar ${cameraType}: ${e.message || e}`));
      }
    },
    ...
    async prepareSelfie(type) {
        await this.prepareCamera(
            `Selfie ${type}`,
            (services) => this.sdk.camera.prepareSelfieCamera(services, SelfieCameraTypes[type]),
            (data) => { },
            (err) => { }
        );
    },
    ...
    async prepareDocs(type, others = false) {
        const documentCameraType = others
         ...
         if (!documentCameraType) {
            this.handleError(new Error(`Invalid DocumentCameraType for: ${type}, others=${others}`));
            return;
         }

        await this.prepareCamera(
            `Document ${type}`,
             (services) => this.sdk.camera.prepareDocumentCamera(services, documentCameraType),
            (data) => { },
            (err) => { }
        );
    },
